...
For this demo I did not go through the checks so for chatbot replace token with the following where it says REPLACE_TOKEN (with one I have included in discord)

Also deployed link is as follows:
```
http://sampleawsbucketsplease.s3-website-us-east-1.amazonaws.com/
```
